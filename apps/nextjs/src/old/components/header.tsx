...
import dayjs from "~/old/utils/dayjs";
import { defaultStyle } from "./button";

export const Header = () => {
  const { query, pathname, push } = useRouter();
  const teamId = z.string().uuid().optional().parse(query.teamId);
    ...
    <header className="flex w-full justify-between bg-zinc-800 p-4">
      ...
      {session?.user && (
        <div className="flex items-center">
          <DropdownMenu.Root>
            <DropdownMenu.Trigger className={clsx(defaultStyle, "px-4 py-2")}>
              {session.user.name}
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content className="flex flex-col">
                <DropdownMenu.Item
                  ...
                  className={clsx(
                    defaultStyle,
                    "rounded-b-none bg-zinc-600 text-center hover:bg-zinc-500",
                  )}
                ...
                <DropdownMenu.Item
                  ...
                  className={clsx(
                    defaultStyle,
                    "rounded-t-none bg-zinc-600 text-center hover:bg-zinc-500",
                  )}
