...
import { useConfirmClick } from "src/hooks/use-confirm-click";
import { z } from "zod";

import { defaultStyle } from "~/old/components/button";
import { api, RouterOutputs } from "~/old/utils/api";
import dayjs, { Dayjs, dayjsLib, displayTime } from "~/old/utils/dayjs";
import { createMyForm } from "~/old/utils/form";
...
const DayRow: React.FC<{
  ...
  return (
    <div className="flex items-center py-1 text-center">
      <h3 className="w-10 text-2xl font-bold">{day.padStart(2, "0")}</h3>
      ...
      <div className="w-4"></div>
      {balanceTime && (
        <div className={clsx("w-16 text-lg font-bold")}>
